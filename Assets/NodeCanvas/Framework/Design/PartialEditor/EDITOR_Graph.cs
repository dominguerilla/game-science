...
namespace NodeCanvas.Framework{
	...
	partial class Graph {
		...
		public void ShowNodesGUI(Event e, Rect drawCanvas, bool firstDraw, Vector2 canvasMousePos, float zoomFactor){
			...
			for (var i = 0; i < allNodes.Count; i++){
				if (firstDraw || drawCanvas.Overlaps(allNodes[i].nodeRect))
			...
			if (GUILayout.Button("File", EditorStyles.toolbarDropDown, GUILayout.Width(50))){
				...
				menu.AddItem (new GUIContent ("Import JSON"), false, ()=>
				{
	                ...
	                if (!string.IsNullOrEmpty(path)){
	                    if ( !this.Deserialize( System.IO.File.ReadAllText(path) )){
				...
				menu.AddItem (new GUIContent ("Export JSON"), false, ()=>
				{
	                ...
	                if (!string.IsNullOrEmpty(path)){
	                    System.IO.File.WriteAllText( path, this.Serialize(true) ); //true for pretyJson
		...
		void ShowBlackboardGUIPanel(){
			...
			var title = blackboard == localBlackboard? "Local Variables" : "Variables";
			if (NCPrefs.showBlackboard){
				GUILayout.BeginArea(blackboardRect, title, (GUIStyle)"editorPanel");
				GUILayout.Space(5);
				var lastSkin = GUI.skin;
				GUI.skin = null;
				BlackboardEditor.ShowVariables(blackboard);
