...
namespace NodeCanvas.Framework{
	...
	abstract public class GraphOwner : MonoBehaviour {
		...
		[HideInInspector]
		public EnableAction enableAction = EnableAction.StartBehaviour;
		///What will happen OnDisable
		[HideInInspector]
		public DisableAction disableAction = DisableAction.StopBehaviour;
		...
		protected void Start(){
			startCalled = true;
			if (enableAction == EnableAction.StartBehaviour)
				StartBehaviour();
		}

		//handle enable behaviour setting
		protected void OnEnable(){
			if (startCalled && enableAction == EnableAction.StartBehaviour)
				StartBehaviour();
		}
		...
		protected void OnDisable(){
			...
			if (disableAction == DisableAction.StopBehaviour)
				StopBehaviour();
